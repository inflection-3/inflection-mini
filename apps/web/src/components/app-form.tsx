...
import { z } from "zod";
import { createAppSchema, createCategorySchema, type Category, type PartnerApplication } from "@mini/types";
...
interface AppFormProps {
  editMode?: boolean;
  defaultData?: PartnerApplication;
  onSuccess?: () => void;
  onCancel?: () => void;
}

export function AppForm({ editMode = false, defaultData, onSuccess, onCancel }: AppFormProps) {
  const [formData, setFormData] = useState<z.infer<typeof createAppSchema>>({
    categoryId: defaultData?.categoryId || "",
    slug: defaultData?.slug || "",
    appName: defaultData?.appName || "",
    appUrl: defaultData?.appUrl || "",
    appDescription: defaultData?.appDescription || "",
    appBadgeLabel: defaultData?.appBadgeLabel || "",
  });
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    setUploadProgress({ step: 'creating', message: editMode ? 'Updating app...' : 'Creating app...' });
    ...
    try {
      ...
      if (editMode && defaultData) {
        ...
        appResult = await new Promise<any>((resolve, reject) => {
      ...
      if (!appResult?.id) {
        throw new Error(`Failed to ${editMode ? 'update' : 'create'} app - no ID returned`);
      }

      // Step 2: Handle avatar upload/removal
      console.log('Avatar file state:', avatarFile, 'Remove existing:', removeExistingAvatar);
      if (avatarFile) {
      ...
      } else if (editMode && removeExistingAvatar) {
        // In edit mode, if user removed existing avatar, we could call a delete endpoint
        // For now, we'll let the backend handle this or implement a delete endpoint later
        console.log('User removed existing avatar');
      }

      // Step 3: Handle banner upload/removal
      console.log('Banner file state:', bannerFile, 'Remove existing:', removeExistingBanner);
      if (bannerFile) {
      ...
      } else if (editMode && removeExistingBanner) {
        // In edit mode, if user removed existing banner, we could call a delete endpoint
        // For now, we'll let the backend handle this or implement a delete endpoint later
        console.log('User removed existing banner');
      }

      // Step 4: Reset form and show success
      setUploadProgress({ step: 'completed', message: `App ${editMode ? 'updated' : 'created'} successfully!` });
      ...
      console.error(`Failed to ${editMode ? 'update' : 'create'} app:`, error);
      setUploadProgress({ step: 'completed', message: '' });
    } finally {
      setIsSubmitting(false);
    }
  }, [validateForm, createAppMutation, updateAppMutation, uploadMutation, formData, avatarFile, bannerFile, isSubmitting, editMode, defaultData, onSuccess]);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <UploadAvatar 
          setAvatarFile={useCallback((file: File | null) => {
            setAvatarFile(file);
            if (file === null && editMode && defaultData?.appLogo) {
              setRemoveExistingAvatar(true);
            } else {
              setRemoveExistingAvatar(false);
            }
          }, [editMode, defaultData?.appLogo])} 
          existingImageUrl={editMode && defaultData?.appLogo ? defaultData.appLogo : undefined}
        />
      </div>
      ...
      <div className="space-y-2">
        ...
        <UploadBanner 
          setBannerFile={useCallback((file: File | null) => {
            setBannerFile(file);
            if (file === null && editMode && defaultData?.bannerImage) {
              setRemoveExistingBanner(true);
            } else {
              setRemoveExistingBanner(false);
            }
          }, [editMode, defaultData?.bannerImage])} 
          existingImageUrl={editMode && defaultData?.bannerImage ? defaultData.bannerImage : undefined}
        />
      </div>
      ...
      {isSubmitting && (
        <div className="space-y-2 p-4 bg-muted rounded-lg">
          ...
          <div className="text-xs text-muted-foreground">
            {uploadProgress.step === 'creating' && 'Step 1 of ' + (avatarFile || bannerFile ? (avatarFile && bannerFile ? '3' : '2') : '1')}
      ...
      <div className="flex gap-2 pt-4">
        ...
        <Button 
          ...
          className="flex-1"
        >
          {isSubmitting ? uploadProgress.message : (editMode ? "Update App" : "Create App")}
        </Button>
      </div>
    </form>
  );
